"""
Harness de evidência do workbook A41 (Etapa 2 — saneamento controlado).
...
Uso (reprodutível pelo auditor):
    ...
    python3 -m pytest audit/area_41_v5/check_a41_v5.py -v
    A41_WORKBOOK=<outro.xlsx> python3 -m pytest audit/area_41_v5/check_a41_v5.py
    python3 audit/area_41_v5/check_a41_v5.py   # grava evidence/*.json
"""
...
DEFAULT_WORKBOOK = HERE / "descritivo_das_variáveis_A41_v5.xlsx"
...
def discount_source_consistency(model) -> dict:
    """
    As linhas da aba 'Forecast A41' são identificadas pela condição dos
    parâmetros retirada_meta_41X (coluna B). Cada desconto_retirada_41X
    deveria vir da linha da sua própria condição.
    """

    condition_by_row = {}
    for e in model["entities"]:
        match = re.fullmatch(r"retirada_meta_(41[a-z])", e["name"])
        if match:
            condition_by_row[int(_CELL.match(e["source_reference"]).group(2))] = match.group(1)

    findings = []
    cells = defaultdict(list)
    for e in model["entities"]:
        match = re.fullmatch(r"desconto_retirada_(41[a-z])", e["name"])
        if not match:
            continue
        cell = _CELL.match(e["source_reference"])
        row = int(cell.group(2))
        cells[e["source_reference"]].append(f"{e['name']}@{e['scope_value']}")
        findings.append(
            {
                "entity": f"{e['name']}@{e['scope_value']}",
                "workbook_row": e["row"],
                "source_reference": e["source_reference"],
                "condition_of_source_row": condition_by_row.get(row),
                "consistent": condition_by_row.get(row) == match.group(1),
            }
        )
    shared = {cell: names for cell, names in cells.items() if len(names) > 1}
    return {"condition_by_row": condition_by_row, "findings": findings, "shared_cells": shared}


def test_41c_41d_source_consistency(model):
    report = discount_source_consistency(model)
    print("\n" + json.dumps(report, ensure_ascii=False, indent=1))
    inconsistent = [f for f in report["findings"] if not f["consistent"]]
    assert inconsistent == [] and report["shared_cells"] == {}, (
        "BLOCKING 41c/41d: fonte declarada contradiz o nome da condição"
    )
...
BLOCKED_ENTITIES = {
    "desconto_retirada_41c@L4_L5", "desconto_retirada_41c@L6_L7",
    "desconto_retirada_41d@L4_L5",
}
...
def traceability(model, materialized) -> list[dict]:
    ...
    for e in model["entities"]:
        ...
        consumes_blocked = e["role"] == "equation" and any(
            d.startswith("desconto_retirada_41") for d in deps
        )
        rows.append(
            {
                "row": e["row"],
                ...
                "validation_status": (
                    "BLOCKING (41c/41d fonte)" if key in BLOCKED_ENTITIES
                    else "PASS (consome 41c/41d BLOCKING)" if consumes_blocked
                    else "PASS"
                ),
            }
        )
...
def main():
    ...
    (out / "a41_v5_evidence.json").write_text(
        json.dumps(evidence, ensure_ascii=False, indent=1, default=str) + "\n", encoding="utf-8"
    )
    print(out / "a41_v5_evidence.json")
